...
#define FACTORY_DEFAULT_RESET_SECONDS 5
static int pressed_limit_seconds = FACTORY_DEFAULT_RESET_SECONDS;
static char reset_cmd[128] = "/sbin/reboot";
static char default_cmd[128] = "/bin/systemctl stop cgminer; /bin/cp /etc/cgminer.conf.factory /config/cgminer.conf";
static int do_events(int fd)
{
	struct input_event ev[64];
	int i, rd;
	fd_set rdfs;
	int pressed = 0, factory = 0;
...
//		select(fd + 1, &rdfs, NULL, NULL, NULL);
		if (stop)
			break;
		if((pressed>0)&&(pressed <= pressed_limit_seconds))
			pressed++;
		else if(pressed >= pressed_limit_seconds) {
...
//			printf("Event: time %ld.%06ld, ", ev[i].time.tv_sec, ev[i].time.tv_usec);
			...
			if ( (type == EV_KEY) && (code == KEY_F24)) {
				if( (ev[i].value == 0) && (pressed >= 1) ) {
#if 0
					if(pressed >= pressed_limit_seconds)
#else
					if(tdiff(&ev[i].time, &pressed_time) > pressed_limit_seconds)
				...
				if ( (ev[i].value == 1) && (pressed == 0) ) {
					pressed = 1;
					pressed_time.tv_sec  = ev[i].time.tv_sec;
					pressed_time.tv_usec = ev[i].time.tv_usec;
				}
			}
		}
		sleep(1); 
...
#if 1
{
	...
	if(argc>1) {
		errno = 0;
		pressed_limit_seconds = strtoul(argv[1], NULL, 0);
		if(errno) return usage();
	}
	if(argc>2)
		strcpy(reset_cmd, argv[2]);
	if(argc>3)
		strcpy(default_cmd, argv[3]);
